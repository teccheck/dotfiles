...
function ytmdl -d "Download music from YTM"
    yt-dlp -f bestaudio --extract-audio --add-metadata --parse-metadata "playlist_index:%(meta_track)s" --output "%(artist)s/%(album)s/%(playlist_index)s %(artist)s - %(title)s.%(ext)s" $argv
end
